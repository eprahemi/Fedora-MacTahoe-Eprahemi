function extract --description 'Extract any archive file — auto-detects format'
    if not set -q argv[1]
        ...
        echo -e "\033[1;30mSupported: .tar.gz/.tgz .tar.bz2/.tbz2 .tar.xz/.txz .tar.zst .tar .gz .bz2 .xz .zst .zip .rar .7z .lz4\033[0m"
        return 1
    end
    ...
    if test -n "$dest"
        if test -d "$dest"
            ...
            echo -e "\033[1;31m❌ Destination not a directory: \033[1;33m$dest\033[0m"
            return 1
        end
    end
