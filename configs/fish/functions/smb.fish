...
function smb --description 'Samba file sharing manager'
    ...
    if test "$argv[1]" = "setup"
        ...
        # Ensure smb.conf exists and is valid — nuke removes it, reinstall may give broken skeleton
        printf "  Verifying smb.conf...\n"
        if not testparm -s 2>/dev/null | head -1 | grep -q '\[global\]'
        ...
        printf "  $C│$N    $N Username:  $W$smb_user$N                                  \n"
        printf "  $C│$N    $N Share:     $W$scope_type ($scope_path)$N                         \n"
        printf "  $C│$N    $N Firewall:  $W samba service$N                            \n"
        printf "  $C│$N    $N SELinux:   $W home dirs enabled$N                        \n"
        printf "  $C╰──────────────────────────────────────────────────────────╯$N\n"
        printf "\n"
    ...
    if test "$argv[1]" = "share"
        ...
        # ── smb share rename <old> <new> ──
            ...
            if test -z "$dir"; or test -z "$name"
                ...
                printf "  Example: smb share rename docs MyDocs\n"
    ...
    if test "$argv[1]" = "data"
        ...
        switch $data_subcmd
            ...
            case '*'
                ...
                printf "  $C┌──────────────────────────────────────────────────────────┐$N\n"
                printf "  $C│$N                                                          \n"
                printf "  $C│$N                    $BOLDG ZIP PASSWORD$N                    \n"
                printf "  $C│$N                                                          \n"
                printf "  $C├──────────────────────────────────────────────────────────┤$N\n"
                printf "  $C│$N                                                          \n"
                printf "  $C│$N  Password:  $BOLDY$W$zip_pass$N                            \n"
                printf "  $C│$N                                                          \n"
                printf "  $C│$N  $Y⚠$N  WARNING: If you close this terminal, the password   \n"
                printf "  $C│$N     is gone forever. You will need to generate a new     \n"
                printf "  $C│$N     export with 'smb data' again.                        \n"
                printf "  $C│$N                                                          \n"
                printf "  $C│$N  To extract:                                              \n"
                printf "  $C│$N    $W unzip $zipname$N                                    \n"
                printf "  $C│$N    Enter password when prompted                           \n"
                printf "  $C│$N                                                          \n"
                printf "  $C└──────────────────────────────────────────────────────────┘$N\n"
                printf "\n"
                ...
                if type -q wl-copy
                    ...
                    printf "  $BOLDG✓$N  $BOLDG Password copied to clipboard.$N\n"
