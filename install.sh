...
prompt_sudoers_entry() {
  if [ -z "${INSTALL_SUDOERS_HINT:-}" ]; then
    ...
    echo -e "  ${CYAN}╠══════════════════════════════════════════════════════════════╣${NC}"
    echo -e "  ${CYAN}║${NC}                                                              ${CYAN}║${NC}"
    echo -e "  ${CYAN}║${NC}  Every time you run a command with ${BOLD}sudo${NC}, Linux asks for your              ${CYAN}║${NC}"
    echo -e "  ${CYAN}║${NC}  password. If you get tired of typing it over and over, you can              ${CYAN}║${NC}"
    echo -e "  ${CYAN}║${NC}  set up ${BOLD}passwordless sudo${NC} — it lets you run sudo commands without               ${CYAN}║${NC}"
    echo -e "  ${CYAN}║${NC}  typing your password every single time.                                      ${CYAN}║${NC}"
    echo -e "  ${CYAN}║${NC}                                                              ${CYAN}║${NC}"
    echo -e "  ${CYAN}║${NC}  This option will ${BOLD}${YELLOW}add a line${NC} at the very end of a special system            ${CYAN}║${NC}"
    echo -e "  ${CYAN}║${NC}  file called ${DIM}/etc/sudoers${NC}. The line will be ${DIM}commented out${NC} (it starts              ${CYAN}║${NC}"
    echo -e "  ${CYAN}║${NC}  with a ${BOLD}#${NC} sign), which means it does ${BOLD}NOTHING${NC} until you manually               ${CYAN}║${NC}"
    echo -e "  ${CYAN}║${NC}  remove that ${BOLD}#${NC} sign yourself. It is perfectly safe and changes nothing             ${CYAN}║${NC}"
    echo -e "  ${CYAN}║${NC}  on your system right now. It is just a ${DIM}convenient shortcut${NC} for later.          ${CYAN}║${NC}"
    echo -e "  ${CYAN}║${NC}                                                              ${CYAN}║${NC}"
    echo -e "  ${CYAN}║${NC}  ${BOLD}If you say yes now:${NC}                                                       ${CYAN}║${NC}"
    echo -e "  ${CYAN}║${NC}  ${DIM}  1.${NC} We add one line at the bottom of /etc/sudoers that looks like:              ${CYAN}║${NC}"
    echo -e "  ${CYAN}║${NC}       ${DIM}# your_username ALL=(ALL) NOPASSWD: ALL${NC}                                    ${CYAN}║${NC}"
    echo -e "  ${CYAN}║${NC}  ${DIM}  2.${NC} The ${BOLD}#${NC} at the start makes it a ${DIM}comment${NC} — it does nothing                ${CYAN}║${NC}"
    echo -e "  ${CYAN}║${NC}  ${DIM}  3.${NC} Later, if you want passwordless sudo, run:                                   ${CYAN}║${NC}"
    echo -e "  ${CYAN}║${NC}       ${BOLD}sudo visudo${NC}                                                          ${CYAN}║${NC}"
    echo -e "  ${CYAN}║${NC}  ${DIM}  4.${NC} Scroll to the bottom and ${DIM}delete the ${BOLD}#${NC}${DIM} at the start of the line${NC}                ${CYAN}║${NC}"
    echo -e "  ${CYAN}║${NC}  ${DIM}  5.${NC} Save and exit — now sudo never asks for your password again                 ${CYAN}║${NC}"
    echo -e "  ${CYAN}║${NC}                                                              ${CYAN}║${NC}"
